...
function dwqa_action_vote( ) {
    ...
    if ($dwqa_user_vote_id!=''){
        if ( ! dwqa_is_user_voted( $post_id, $point, $dwqa_user_vote_id ) ) {
            ...
            // Update vote point
            dwqa_update_vote_count( $post_id,$point );
 ...
 * @return void
 */
function dwqa_update_vote_count( $post_id,$point) {
    if ( ! $post_id ) {
        global $post;
        $post_id = $post->ID;
    }
    //Getting user total vote count
    $votes= get_post_meta( $post_id, '_dwqa_votes', true );
    
//For checking user vote is upward or downward
    if($point>0){
        $total= ($votes)+1;
    }
    else if($point<0){
